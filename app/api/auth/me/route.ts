...
export async function GET(req: NextRequest) {
  return withAuth(async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      let foundUser: Trainer | Client | undefined = mockTrainers.find((t: any) => t.id === user.id);
      let userType: 'trainer' | 'client' = 'trainer';

      if (!foundUser) {
        foundUser = mockClients.find((c: any) => c.id === user.id);
        userType = 'client';
      }
...
export async function PUT(req: NextRequest) {
  return withAuth(async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      // Поиск пользователя
      let userIndex = mockTrainers.findIndex((t: any) => t.id === user.id);
      let userType: 'trainer' | 'client' = 'trainer';

      if (userIndex === -1) {
        userIndex = mockClients.findIndex((c: any) => c.id === user.id);
        userType = 'client';
      }
      ...
      if (body.email && body.email !== user.email) {
        const emailExists = [
          ...mockTrainers,
          ...mockClients
        ].some((u: any) => u.email === body.email && u.id !== user.id);
  ...
  return withAuth(async (authenticatedReq: AuthenticatedRequest) => {
    try {
      ...
      if (userType === 'trainer') {
        ...
        if ((trainer as any).activeClients > 0) {
          return NextResponse.json(
