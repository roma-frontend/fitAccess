    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">Не удалось загрузить данные аналитики</p>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Подробная статистика и отчеты системы</p>
          </div>
        ...
        <Card className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold mb-1">{data.users.total.toLocaleString()}</div>
                <div className="text-blue-100">Всего пользователей</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold mb-1">{data.products.total}</div>
                <div className="text-blue-100">Продуктов</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold mb-1">{(data.revenue.total / 1000000).toFixed(1)}М</div>
                <div className="text-blue-100">Выручка (₽)</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold mb-1">{data.activity.totalSessions}</div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-600">{data.users.total.toLocaleString()}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">{data.users.active.toLocaleString()}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-600">{data.users.new}</p>
                  </div>
              ...
              <div className="h-64 relative">
                <svg className="w-full h-full" viewBox="0 0 400 200">
                  ...
                  <polyline
                    ...
                    stroke="#3b82f6"
                    strokeWidth="3"
                    points={data.users.registrationTrend.map((point, index) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <Card className="border-l-4 border-l-blue-500">
              <CardContent className="p-6 text-center">
                <div className="text-2xl font-bold text-blue-600">{data.products.total}</div>
            ...
            <CardContent>
              <div className="space-y-4">
                {data.revenue.byProduct.map((product, index) => {
                  const maxRevenue = Math.max(...data.revenue.byProduct.map(p => p.revenue));
              ...
              <div className="h-64 relative">
                <svg className="w-full h-full" viewBox="0 0 400 200">
                  ...
                  <polygon
                    ...
                    points={`0,200 ${data.revenue.trend.map((point, index) => {
                  ...
                  <polyline
                    ...
                    stroke="#10b981"
                    strokeWidth="3"
                    points={data.revenue.trend.map((point, index) => {
                      const x = (index / (data.revenue.trend.length - 1)) * 400;
                      const maxValue = Math.max(...data.revenue.trend.map(d => d.amount));
                      const y = 200 - (point.amount / maxValue) * 180;
                      return `${x},${y}`;
                ...
                <div className="absolute bottom-0 left-0 w-full flex justify-between text-xs text-gray-500 -mb-6">
                  {data.revenue.trend.map((point, index) => (
                    index % 2 === 0 && (
                      <span key={index}>
                        {new Date(point.date).toLocaleDateString('ru', { month: 'short', day: 'numeric' })}
                      </span>
                    )
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Button 
              ...
              onClick={() => {
                ...
                const csvData = Object.entries(data.users.byRole).map(([role, count]) => [role, count]);
                const csv = [['Role', 'Count'], ...csvData].map(row => row.join(',')).join('\n');
            ...
            <Button 
              ...
              onClick={() => {
                ...
                const csvData = Object.entries(data.products.byCategory).map(([category, count]) => [category, count]);
                const csv = [['Category', 'Count'], ...csvData].map(row => row.join(',')).join('\n');
                const blob = new Blob([csv], { type: 'text/csv' });
                const url = URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
            ...
            <Button 
              ...
              onClick={() => {
                // CSV экспорт выручки
                const csvData = data.revenue.byProduct.map(product => [product.name, product.revenue]);
                const csv = [['Product', 'Revenue'], ...csvData].map(row => row.join(',')).join('\n');
                const blob = new Blob([csv], { type: 'text/csv' });
                const url = URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.href = url;
